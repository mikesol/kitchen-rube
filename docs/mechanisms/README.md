# Mechanisms

Mechanism dossiers are provisional records. Some will become final parts of the machine; others will be retired after experiments or layout changes.

Create one dossier per meaningful mechanism idea, proof, or integrated assembly. Copy `mechanism-template.md` and update the status as the mechanism evolves.

Phase 2 dossiers are composition anchors. They identify mechanisms that shape the wall: trigger area, routing decisions, stored energy, marble travel, and gong-court strikes.

Phase 3 stocks the pantry. Broad ideas, wild variants, prior-art prompts, kitchen-tool transfers, route identities, and cross-category hybrids belong in `pantry.md`.

Do not promote new full dossiers early in Phase 3. Use the pantry to get ideas out of the system first, then run an end-of-phase promotion pass for the strongest or riskiest candidates.

When a pantry entry depends on hidden geometry, multiple states, or sequence, mark its clarity status before it turns into shorthand. Rewrite confusing entries in plain mechanical language before using concept art; images should clarify a mechanism, not compensate for vague prose.

Use `pantry-prompts/` for detailed subject briefs when a mechanism needs concept art or prompt-level clarification. These briefs should make the mechanism grokable without pretending to be CAD, final layout, or proof that the mechanism will work.
