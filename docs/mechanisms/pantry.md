...
- **Jacquard meal-card selector** - a small flat wooden card, punched with a sparse pattern of holes, slides into a frame behind a matrix of vertical pins. The holes correspond to the chosen meal's route; pins above those holes drop through the card to engage a latch or cam-follower below, while pins above blank card stay raised and inert. Three cards — breakfast, lunch, dinner — live in a flip-stack near the trigger column, and choosing the meal is choosing which card to slot in. The reference is Joseph-Marie Jacquard's loom-card mechanism (Lyon, 1804), built on Bouchon's perforated paper roll (1725) and Falcon's chained cards (1728), in which punched cards lifted specific warp threads on each pick of the shuttle and so wove the pattern. The fantasy is a kitchen weaving its own meal — the program for what fires is something you can hold in your hand and look through at the light. `trigger`, `shared`
...
- **Signal-box meal lever frame** - three upright levers sit in a shallow frame, one per meal. Pulling any lever slides a locking bar behind the frame so the other two lever slots are blocked, while the pulled lever also aligns that meal's first gate. The fantasy is breakfast, lunch, and dinner being dispatched like railway routes. `trigger`, `shared`, `latch`, `clarity: expanded`
- **Cash-register meal keys** - three broad keys sit side by side. Pressing one key raises a visible meal flag, locks out the other two keys, and only then allows a common launch drawer or pin to move. The fantasy is the kitchen ringing up breakfast, lunch, or dinner as a real mechanical transaction. `trigger`, `shared`, `latch`, `clarity: expanded`
- **Rotary-phone pulse selector** - the user puts a finger in a breakfast, lunch, or dinner hole and turns a spring-return dial to a stop. As the dial returns, a ratchet counts one, two, or three pulses into a stepped gate, then the final return releases the launch. The fantasy is ordering a meal by dialing it. `trigger`, `sequencer`, `delay`, `rotary`, `clarity: expanded`
- **Coin-slot meal tokens** - three meal tokens have visibly different diameters or edges and enter matching slots. The falling token pulls a route latch as it descends, then lands in a return cup so the selection can be reset without hunting for loose parts. The fantasy is a vending machine that accepts appetite as currency. `trigger`, `shared`, `sorter`, `clarity: expanded`
- **Bitted spoon key** - a spoon handle carries two or three blunt notches. Inserting and turning it lifts a matching set of little blockers, so only the corresponding meal latch can move. The spoon bowl remains the handle; the coded stem does the work. `trigger`, `compact`, `kitchen-object`, `clarity: expanded`
- **Cookbook page-notch selector** - the cookbook opens to breakfast, lunch, or dinner. Each page has a notch in a different horizontal position; when the cover is pressed, a follower pin drops through the notch and shifts the route gate to that page's meal. The page is not signage; it is the cam plate. `trigger`, `shared`, `clarity: expanded`
- **Pantry scale ballast selector** - placing one of three labeled ingredient weights on a small balance scale tilts the beam to a repeatable stop. Each stop height lines up a different gate or cable eye. Lunch can own the most balanced version, but the core move is shared. `trigger`, `shared`, `lunch`, `clarity: expanded`
- **Measuring-cup ballast pour** - the user pours beans, rice, or dry pasta into a hanging cup up to a breakfast, lunch, or dinner fill line. The cup descends only when the chosen fill line has enough mass, pulling a latch and then dumping into a catch trough for reset. `trigger`, `delay`, `reset`, `clarity: expanded`
- **Butter-dish slide bolt** - sliding a butter-dish lid sideways moves a shallow bolt. The bolt crosses one of three route holes, and the selected meal tab decides which hole is unblocked before the lid reaches the launch notch. `trigger`, `compact`, `kitchen-object`
- **Mug-hook lift selector** - three mugs hang from springy hooks. Removing one mug lets that hook rise under a small counterweight, pulling its route latch; putting the mug back resets the hook. The gesture is choosing a cup, and the missing cup is the visible selected state. `trigger`, `reset`, `kitchen-object`, `clarity: clear`
- **Rolling-pin drawbar** - a rolling pin acts as a long horizontal handle. Pulling it draws a perforated bar through three possible cable loops; the selected meal peg catches only one hole, so one large hand motion becomes the chosen route pull. `trigger`, `wide`, `transfer`, `clarity: expanded`
- **Pepper-mill crank armer** - twisting a pepper-mill body turns a hidden ratchet that lifts a small start weight one tooth at a time. Breakfast can use one quick click, lunch two, dinner a slow full turn; the final tooth drops the weight into the chosen launch. `trigger`, `rotary`, `sequencer`, `dinner`, `clarity: expanded`
- **Can-opener indexing crank** - a manual can-opener handle turns a small gear that walks a rack through breakfast, lunch, and dinner detents. A separate squeeze on the handles fires only when the rack is seated, turning a familiar clamp-and-crank object into a selector with a launch interlock. `trigger`, `rotary`, `gear`, `latch`, `clarity: expanded`
- **Apron-string two-hand arming pull** - two short cords must be pulled apart at the same time, like tightening an apron. Equal tension lifts a safety bar; uneven or one-handed tension leaves the dinner release blocked. The object language is domestic, but the causal role is an intentional two-hand interlock for higher-energy starts. `trigger`, `stored-energy-risk`, `safety`, `dinner`, `clarity: expanded`
- **Cutlery-tray shuttle** - a shallow tray slides left and right under a fixed drop point. Three utensil lanes cut into the tray line up with breakfast, lunch, or dinner; the launch ball falls only into the lane currently under the drop. The tray is both selector and first track. `trigger`, `shared`, `sorter`, `track`, `clarity: clear`
- **Order-spike ticket selector** - a paper or wooden ticket for the chosen meal slides onto a vertical restaurant-order spike. Each ticket has a different notch along one edge; pushing it down lets one follower drop while the other followers ride on solid material. The ticket is a physical program, not a label. `trigger`, `shared`, `latch`, `clarity: expanded`
- **Mixing-bowl tilt cradle** - a bowl sits in a three-detent cradle. Tilting it toward breakfast, lunch, or dinner rolls a captive marble inside the rim to a matching exit hole, where it drops into the selected route. The bowl's tilt is both gesture and gravity switch. `trigger`, `shared`, `sorter`, `clarity: expanded`
- **Egg-carton detent row** - three egg-cup depressions act as plungers. Pressing one cup lowers its pin and raises a cross-lock under the other two cups, so the first press wins and physically blocks double selection. The pressed cup stays down until the reset rail lifts the row. `trigger`, `breakfast`, `compact`, `latch`, `clarity: expanded`
- **Placemat hole mask** - a sliding placemat carries three sets of cutouts. Pulling it to breakfast, lunch, or dinner lets only the matching pins fall through, while the uncut parts of the mat hold the other pins up. It is a table-setting version of a stencil gate. `trigger`, `shared`, `compact`, `clarity: expanded`
- **Kitchen scale over/under fork** - instead of three exact weights, a balance beam has two trip points: below the first point routes breakfast, between points routes lunch, above the second routes dinner. It turns weight choice into a three-way analog selector and makes wrong mass visibly ambiguous rather than silently failing. `trigger`, `sorter`, `clarity: needs-prior-art`
...
- **Meal-ratio gear train** - a brass selector lever with three detented notches engages one of three idler gear pairs between a shared input shaft and a shared output shaft, so the same lever pull moves the breakfast route quickly, the lunch route at a middling pace, and the dinner route slowly and ponderously. The reference is the Antikythera mechanism (c. 100 BCE), whose single hand-crank drove the Metonic nineteen-year cycle, the Saros eclipse cycle, and several planetary periods from one input through differently geared trains; closer to the kitchen in scale, Holtzapffel's ornamental-turning lathes (London, from 1794) selected feed-rate ratios by inserting interchangeable brass change-wheels into the train. The wall version keeps the selectable-ratio idea and discards real power transmission — the train only re-times. The fantasy is a single twist of the wrist that breakfast finds bright, lunch finds reasonable, and dinner finds heavy. `gear`, `shared`, `delay`
...
- **Segment gear hesitation** - a wheel carries gear teeth around only part of its rim. As the wheel turns, its toothless arc passes silently across the follower pinion and nothing moves; then the toothed sector arrives and the follower jerks through its share of motion in one tight burst. The reference is the *mutilated gear* of early calculating machines: Wilhelm Schickard's calculating clock (1623) used a one-tooth carry wheel between digit columns, and Pascal's Pascaline (1642) replaced it with the elegant *sautoir*, a weighted lever cocked by the previous digit's rotation and gravity-dropped onto the next column. The fantasy is a meal arming itself for five long seconds of visible nothing, then committing in a single audible click. `gear`, `delay`
...
- **Snail cam dinner dwell** - a slow horizontal cam, cut as a spiral with one sharp cliff, turns under a follower arm. Through most of the rotation the follower rides at the snail's outer radius and the dinner latch stays closed; as the cam turns past its cliff, the follower drops suddenly and trips the release. The reference is Edward Barlow's rack-and-snail clock strike (1676), where the snail is the schedule that tells the hour-bell how many notes to play — the moment of the rack-tail's fall off the snail's edge is called the *warning*, and on this wall it is the moment dinner is finally allowed to begin. The fantasy is a slow brass schedule that has been holding its tongue all evening and now lets go. `rotary`, `delay`, `dinner`
...
- **Pulley multiplication theater** - a dramatic vertical column of six or eight pulleys, cords zigzagging between them in the manner of a Vitruvian polyspaston crane (*De Architectura* X.2, c. 30 BCE), is reduced to lifting one small latch by exactly one inch. The joke is mechanical disadvantage staged as spectacle: where the Roman polyspaston let four winch-men lift three thousand kilos and Archimedes (per Plutarch's *Marcellus*) is said to have hauled a laden ship across Syracuse harbor on a single rope, this rig spends eight rope falls to move a tab the width of a finger. Brunelleschi's *paradiso* at San Felice in Piazza (1430s) charmed Florentine audiences precisely because they could see the ropes; this entry borrows that audience contract. Its deliberate foil is **Hidden mechanical advantage**, which inverts the joke; together they make the leverage grammar legible — ropes and levers mean whatever the route author wants the audience to feel about effort, not raw power.
...
- **Geneva recipe wheel** - a driver pin briefly enters one of four slots in a recipe-card wheel and turns it exactly one station; between hits, a convex stop on the driver disc bolts the wheel shut so it cannot drift. The reference is the Geneva stop cut into Swiss watch barrels in the seventeenth and eighteenth centuries to prevent over-winding of a mainspring; the same indexing was independently re-discovered in 1896 by early film projectors (Oskar Messter, Max Gliewe, Robert William Paul's Theatrograph) so that each frame would be held still in the gate while the shutter opened. Each ball impact or lever stroke advances the visible program by one card; between cards, the program is locked. The fantasy is a film projector pulling a meal one frame at a time. `sequencer`, `clarity: expanded`, `art: ghosted-motion`
...
- **Camshaft of appetizers** - a horizontal shaft carries three or four cam lobes spaced along its length, each cut to a different profile — a kidney, a cloud, a crown. The shaft is indexed in clicks, not free-running. At each click, one lobe lifts a follower that drops into its own small self-contained vignette below — a captured marble runs a short loop, a spoon tips a thimble, a flag pops up and salutes. When that vignette finishes, its own output turns the shaft one click forward, presenting the next lobe and waking the next vignette. The reference is the Swiss automata tradition: Pierre and Henri-Louis Jaquet-Droz's Writer (Neuchâtel, 1774) translated a stack of forty cam edges into the pen-path of a small boy writing a sentence, and Henri Maillardet's Draughtsman-Writer (c. 1800) used seventy-two brass cams to produce four drawings and three poems. The pleasure on this wall is amuse-gueules — three or four tiny mechanical courses, each a little universe, served in sequence by a single shaft. `dinner`
- **Bell-crank relay bank** - a cluster of L-shaped pivot levers crowded into a small panel, each one taking a pull on one arm and returning it as a push at right angles on the other. A horizontal tug becomes a vertical pull, then a sideways latch release, then another vertical pull, so a dense neighborhood of routes shares a single linkage field instead of drawing long separate tracks. The reference is the Victorian servant-call bell system: the bell-crank gets its name from the L-shaped iron lever that hid inside parlor walls, turning a parlor cord through three or four right-angle corners before tugging a labeled bell over the kitchen door. On this wall the bank is exposed and the routing is the show, where the original was meant to disappear into plaster. `route-field`, `clarity: expanded`, `art: single-frame`
...
- **Flyball governor gate** - two small weights pivot outward on hinged arms from a vertical spindle as it spins, driven by a falling weight or a hand crank. Below a threshold speed the weights hang low and a latch stays closed; above it, the weights fly outward and lift a collar that unhooks the latch. The reference is Thomas Mead's 1787 windmill governor (British Patent No. 1628), borrowed and made famous a year later as the brass-balled centrifugal governor on Boulton and Watt's steam engines; the mathematics had already been worked out by Christiaan Huygens in 1659 as the conical pendulum. The wall version keeps only the threshold-trigger and discards the feedback loop — the route earns its release by reaching gong-speed once, not by holding it. The fantasy is a steam-age machine making a deliberate decision about whether it is ready. `delay`, `stored-energy-risk`
...
- **Astronomical-clock figure carousel** - a small horizontal turntable, hidden behind a proscenium window cut into the wall panel, carries three carved chef-figures spaced equally around its rim — an egg-flipper, a sandwich-stacker, a roast-carver. The meal selector indexes the turntable so that exactly one chef is presented in the window when the route fires; as he passes his station, his outstretched arm brushes a latch and trips the next stage. The reference is the procession-of-figures tradition of medieval astronomical clocks: the Walk of the Apostles at Prague's Orloj (Mikuláš of Kadaň with Jan Šindel, 1410), the noon Apostles parade at Strasbourg Cathedral (1354, rebuilt 1843), and the jacquemart bell-strikers of Notre-Dame de Dijon (1383, installed by Philip the Bold). The fantasy is a cathedral clock reduced to three small chefs, each one stepping briefly into view to do his single mechanical job. `kitchen-object`, `shared`
...
- **Servant-bell indicator board** - a small vertical panel hung beside the gong court carries three coil-sprung bells, each labeled with a meal name and each rung by its own cord through a bell-crank relay from upstream in the route. When a route fires, the corresponding bell trembles for several seconds on its spring mount and a small painted flag drops in its window, announcing which meal the wall is presently serving. The reference is the Victorian and Georgian servants' annunciator: a labeled bell board hung in the kitchen passage of a country house, with each pull-cord routed through wall cranks from a different room and each bell sprung so it kept moving after its strike, giving a servant arriving late time to read which one had rung. This wall borrows that mercy. The fantasy is the kitchen passage of a country house — except the rooms calling for service have been replaced by meals announcing themselves. `reset`, `gong-court`
...
- **Funnel orbit to ratchet stepper** - a ball circles inside a shallow funnel-bowl, slowly losing height with each pass. Once per orbit, the ball brushes a single bent-wire lever at the rim, advancing a small brass star-wheel by one tooth; after seven or eight orbits the star-wheel's notch finally aligns and a latch falls. The ball eventually drops through the funnel's center exit, but by then its work of counting is already done. The reference is the charity Spiral Wishing Well (Steve Divnick, Dayton, 1985), in which a coin spirals down a parabolic funnel to a central hole — except here the descent pays a time-toll rather than a coin-toll; the deeper ancestor is Ctesibius of Alexandria's clepsydra (c. 250 BCE), the earliest documented device that converted slow continuous motion into discrete counted ticks. The fantasy is a marble searching for the drain while it pays its way out one tooth at a time. `delay`
