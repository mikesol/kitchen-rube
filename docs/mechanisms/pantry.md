...
- **Weight-and-pulley theater** - dinner pulleys must cost something real. The reference is Giacomo Torelli's pole-and-chariot rigging at Venice's Teatro Novissimo (1641) and the hemp-house tradition that ran from Renaissance court theatres through Edwardian playhouses until the Vienna Burgtheater's 1888 counterweight system: ropes, sandbag counterweights, and brass fairleads carrying real load in plain view. A dinner pulley network earns its complexity only when a hanging counterweight rises an honest inch, or six visible cords drag one small latch open with effort that obviously exceeds the latch's needs — the earnest foil to **Pulley multiplication theater**, which is the same image staged as gag instead of as labor. The audience should leave feeling that dinner takes work.
